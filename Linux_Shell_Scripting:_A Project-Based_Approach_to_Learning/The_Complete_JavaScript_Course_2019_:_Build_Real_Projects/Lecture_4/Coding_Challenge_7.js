...
(function(){
  ...
  Question.prototype.check_answer = function(user_answer,players_points){
    if(user_answer > (this.choose_answer.length-1)){
      console.log("Please add an valid number!"+" "+"current score is "+ players_points(false));
    }

    else if (this.answer === this.choose_answer[user_answer]){
      console.log("You are correct "+ "and your points is: "+players_points(true));
    }
    else if(user_answer === 'exit'){
      game_playing = false;
      console.log("Your final points is: "+players_points(false));
    }
    else{
      console.log("You are wrong! "+ "current points is "+players_points(false));
    }
  }
  ...
  //Inistiate points and keep a check on it.
  ...
  //Generate game
  function generate_game(players_points){
  ...
  quest.check_answer(check,players_points);
  }

var players_points = check_point();
while(game_playing){
generate_game(players_points);
}
})();
